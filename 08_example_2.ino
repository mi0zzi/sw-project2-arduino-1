...
void loop() { 
  ...
  else if ((distance >= 150 && distance <= 160) || (distance >= 240 && distance <= 250)) {
    analogWrite(PIN_LED, 127);  // LED 50% brightness
  } 
  else if (distance <= 200) {  
    analogWrite(PIN_LED, (int)((200 - distance) * (255.0 / 100.0)));
  } 
  else { 
    analogWrite(PIN_LED, (int)((distance - 200) * (255.0 / 100.0)));
  }

  // output the distance to the serial port
  Serial.print("Min:");        Serial.print(MIN_DISTANCE);
  Serial.print(", distance:"); Serial.print(distance);
  Serial.print(", Max:");       Serial.print(MAX_DISTANCE);
  Serial.println("");
